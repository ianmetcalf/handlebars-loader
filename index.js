...
module.exports = function(source) {
  ...
  class JavaScriptCompiler extends hb.JavaScriptCompiler {
    ...
    nameLookup(parent, name, type) {
      ...
      case 'partial':
        ...
        if (!foundStuff[key]) {
          ...
          return `require(${ loaderUtils.stringifyRequest(loaderApi, foundStuff[key]) })`;
        }
      ...
      case 'helper':
        ...
        if (!foundStuff[key]) {
          ...
          return `__default(require(${ loaderUtils.stringifyRequest(loaderApi, foundStuff[key]) }))`;
        }
    ...
    pushString(value) {
      if (inlineRequires && inlineRequires.test(value)) {
        return this.pushLiteral(`require(${ loaderUtils.stringifyRequest(loaderApi, value) })`);
      }
    ...
    appendToBuffer(str) {
      ...
      if (inlineRequires && str.indexOf && str.indexOf('"') === 0) {
        ...
        str = fastreplace(str, replacements, match => (
          `" + require(${ loaderUtils.stringifyRequest(loaderApi, match) }) + "`
        ));
      }
  ...
  (function compile() {
    ...
    try {
      if (source) {
        template = hb.precompile(source, opts);
      }
    } catch (err) {
      return loaderAsyncCallback(err);
    }

    return asyncLib.each(Object.keys(foundStuff), (key, callback) => {
      if (foundStuff[key]) {
        return callback();
      }
